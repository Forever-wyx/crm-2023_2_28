...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#saveBoundBtn").click(function () {
			//获取选中的市场活动的id
			let ids = $("#boundTxt input:checked");
			//表单验证
			if (ids.size() == 0){
				alert("请选择要关联的活动");
				return;
				...
				success:function (data) {
					if (data.code == "1"){
						...
						$.each(data.retData,function () {
							...
							htmlStr += "<td><a clueRemarkId=\"" + this.id + "\" href=\"javascript:void(0);\"  style=\"text-decoration: none;\"><span class=\"glyphicon glyphicon-remove\"></span>解除关联</a></td>"
							htmlStr += "</tr>"
						})
						//刷新列表
		...
		$("#activityTbody").on("click","a",function () {
			if (window.confirm("确定解除关联")){
				//获取参数
				let activityId = $(this).attr("clueRemarkId");
				$.ajax({
...
<%--							<tr>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		<div class="page-header">
			<h3>${clue.fullname} <small>${clue.appellation}</small></h3>
		</div>
		...
		<c:forEach items="${clueRemarkList}" var="clueRemark">
			<div class="remarkDiv" style="height: 60px;">
				...
				<div style="position: relative; top: -40px; left: 40px;" >
					...
					<font color="gray">线索</font> <font color="gray">-</font> <b>${clue.fullname}${clue.appellation}-${clue.company}</b> <small style="color: gray;"> ${clueRemark.editFlag == '1'?clueRemark.createTime:clueRemark.editTime} 由${clueRemark.editFlag == '1'?clueRemark.createBy:clueRemark.editBy}${clueRemark.editFlag == '1'?'创建':'修改'}</small>
					...
					<tbody id="activityTbody">
						<c:forEach items="${activityList}" var="activity">
							<tr id="tr_${activity.id}">
								...
								<td><a clueRemarkId="${activity.id}" href="javascript:void(0);"  style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>
							</tr>
						</c:forEach>
<%--						<tr>
							<td>发传单</td>
			...
			<div>
				<a id="boundActivity" href="javascript:void(0);" style="text-decoration: none;"><span class="glyphicon glyphicon-plus"></span>关联市场活动</a>
			</div>
		</div>
	</div>
